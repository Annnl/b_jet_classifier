...
Names = ["met_pt","bjet_E","el_E","mu_E","bjet_pt","el_pt","mu_pt","bjet_eta","el_eta","mu_eta","bjet_phi","el_phi","mu_phi","met_phi"]
Names_top = []
hist = []
hist_top = []
...
for i in range(len(Names)):
	if i <= 7:
		hist.append(ROOT.TH1D(Names[i],Names[i],40,0,1000))
		hist[i].Sumw2()
		hist_top.append(ROOT.TH1D(Names_top[i],Names_top[i],40,0,1000))
		hist_top[i].Sumw2()
	else:
		hist.append(ROOT.TH1D(Names[i],Names[i],40,-4,4))
		hist[i].Sumw2()
		hist_top.append(ROOT.TH1D(Names_top[i],Names_top[i],40,-4,4))
		hist_top[i].Sumw2()

myfile = ROOT.TFile.Open("3j3b_coor.root","RECREATE")
...
for i in range(len(Names)):
	mytree_sig.Branch(Names[i],leaf[i],Names[i]+"/F")
mytree_sig.Branch("weights",totws,"weights/F")
mytree_sig.Branch("origin",topFs,"origin/I")
mytree_sig.Branch("number",number,"number/I")
	...
	for j in range(tree.GetEntries()):
		...
		for k in range(len(mass_bjet)):
			...
			for g in range(len(coll)):
				if origin[k] == 4:
					hist_top[g].Fill(coll[g],w)
				else:
					hist[g].Fill(coll[g],w)
			mytree_sig.Fill()
